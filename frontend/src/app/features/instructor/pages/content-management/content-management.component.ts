          ...
          @for (module of modules(); track module.id) {
            <mat-card class="surface-card">
              ...
              <mat-card-content>
                <div class="chip-row">
                  <mat-chip-set>
                    <mat-chip [highlighted]="module.status === 'published'">{{ module.status }}</mat-chip>
                    @if (module.is_preview) {
                      <mat-chip>Preview</mat-chip>
                    }
                  </mat-chip-set>
                </div>
    ...
    this.instructorPortalService.listMyCourses()
      ...
      .subscribe({
        next: (response) => {
          ...
          const firstCourseId = response.items[0]?.id ?? '';
          this.courseForm.patchValue({ course_id: firstCourseId });
        },
